...
def extract_low_confidence_chains(
    ...
    while i < len(amino_groups):
        ...
        while i + j < len(amino_groups):
            ...
            if sum(amino_groups[k][1] for k in chain) / len(chain) > MIN_CONFIDENCE_THRESHOLD:
                chain.pop()
                break
...
def run_vqe(main_chain: str) -> list[tuple[float, float, float]]:
    ...
    protein_folding_problem = ProteinFoldingProblem(peptide, interactions, penalty_terms)
...
def translate_to_mean(points_a: list[tuple[float, float, float]], mean_b: tuple[float, float, float]) -> list[tuple[float, float, float]]:
...
def apply_updated_amino_acid_coordinates(
    ...
    for idxs, new_coordinates in zip(idxs_list, new_coordinates_list):
        current_amino_chain_coordinates = [mean([a[1] for a in amino_groups[i][2]]) for i in idxs]
        new_amino_chain_coordinates = translate_to_mean(new_coordinates, mean(current_amino_chain_coordinates))

        for amino_idx, new_amino_acid_coordinate in zip(idxs, new_amino_chain_coordinates):
            current_element_coordinates = [a[1] for a in amino_groups[amino_idx][2]]
            new_element_coordinates = translate_to_mean(current_element_coordinates, new_amino_acid_coordinate)

        # print(amino_groups)
        # print(new_element_coordinates)
        old_element_coordinates = [a[1] for i in idxs for a in amino_groups[i][2]]
        print(old_element_coordinates)
        old_element_coordinate_1 = amino_groups[idxs[0] ][2][-1][1]
        old_element_coordinate_2 = amino_groups[idxs[-1]][2][0 ][1]
        print(old_element_coordinate_1, old_element_coordinate_2)
        trans_rot_args = optimize_new_element_coordinates(
            old_element_coordinate_1,
            old_element_coordinate_2,
            new_element_coordinates,
        ).x

        print(trans_rot_args)
        new_element_coordinates = translate_and_rotate(new_element_coordinates, *trans_rot_args)

        # print(new_element_coordinates)

        for amino_idx, new_amino_acid_coordinate in zip(idxs, new_amino_chain_coordinates):
            amino_idx_offset = first_amino_idx + sum(len(amino_groups[i][2]) for i in range(amino_idx))
...
def run_quantum_folding(pdb_file: str) -> str:
    ...
    pdb_content = apply_updated_amino_acid_coordinates(pdb_content, amino_groups, low_confidence_chains, amino_acid_coordinates_list)
...
def optimize_new_element_coordinates(actual_endpoint_1, actual_endpoint_2, new_element_coordinates):
    def f(args):
        optimized_element_coordinates = translate_and_rotate(new_element_coordinates, *args)
        return error(actual_endpoint_1, actual_endpoint_2, optimized_element_coordinates[0], optimized_element_coordinates[-1])

    ub = 30
    lb = -ub
    bounds = [(lb, lb, lb, 0, 0, 0), (ub, ub, ub, 2 * np.pi, 2 * np.pi, 2 * np.pi)]

    return least_squares(f, (0, 0, 0, 0, 0, 0), bounds=bounds, verbose=1, max_nfev=10000)


def translate_and_rotate(points, trans_x, trans_y, trans_z, rot_x, rot_y, rot_z):
...
def error(actual_endpoint_1, actual_endpoint_2, predicted_endpoint_1, predicted_endpoint_2):
    return sum((actual_endpoint_1[i] - predicted_endpoint_1[i]) ** 2 + (actual_endpoint_2[i] - predicted_endpoint_2[i]) ** 2 for i in range(3))
